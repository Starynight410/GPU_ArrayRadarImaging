/*****************************************************************************
    ...
    Project: 综合孔径阵列雷达成像_v2 2024_12_09
    ================
    ...
    nvcc -o ArrayImaging_v2 ArrayImaging_v2.cu --ptxas-options=-v --use_fast_math -lcublas -lcufft
    ./ArrayImaging
...
int main()
{
    ...
    double txPos_full_x[64];
    double txPos_full_y[64]; 
    double rxPos_full_x[64];
    double rxPos_full_y[64]; 
    ...
    // 分配设备内存
    cudaMalloc(&d_txPos_full_x, 64 * sizeof(double));
    cudaMalloc(&d_txPos_full_y, 64 * sizeof(double));
    cudaMalloc(&d_rxPos_full_x, 64 * sizeof(double));
    cudaMalloc(&d_rxPos_full_y, 64 * sizeof(double));
    ...
    // 将数据从主机复制到设备
    cudaMemcpy(d_txPos_full_x, txPos_full_x, 64 * sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(d_txPos_full_y, txPos_full_y, 64 * sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(d_rxPos_full_x, rxPos_full_x, 64 * sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(d_rxPos_full_y, rxPos_full_y, 64 * sizeof(double), cudaMemcpyHostToDevice);
    ...
    for(int i = 0; i < Naz_net; i++)
    {
        ...
        {   
            ...
            for (int k = 0; k < Na; k++)        // 中间结果索引k
            {
                ...
                {
                    T_sence_left[k].x += (A_Comp[j*Naz_net*Na+i*Na+k2].x * R_sigdata[k2][k].x + \
                                          A_Comp[j*Naz_net*Na+i*Na+k2].y * R_sigdata[k2][k].y);     // [i,j]网格处
                    T_sence_left[k].y += (A_Comp[j*Naz_net*Na+i*Na+k2].x * R_sigdata[k2][k].y - \
                                          A_Comp[j*Naz_net*Na+i*Na+k2].y * R_sigdata[k2][k].x);     // [i,j]网格处
                }
            }
            ...
            for (int k = 0; k < Na; k++)        // 中间结果索引k
            {
                T_sence.x += (T_sence_left[k].x * A_Comp[j*Naz_net*Na+i*Na+k].x - \
                              T_sence_left[k].y * A_Comp[j*Naz_net*Na+i*Na+k].y);
                T_sence.y += (T_sence_left[k].x * A_Comp[j*Naz_net*Na+i*Na+k].y + \
                              T_sence_left[k].y * A_Comp[j*Naz_net*Na+i*Na+k].x);   
