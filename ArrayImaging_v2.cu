/*****************************************************************************
    ...
    Project: 综合孔径阵列雷达成像_v2 2024_05_26
    ================
...
const int Row = 256;
const int Col = 256;
const int Naz_net = 240;
const int Nay_net = 240;
const int Na = 256;

__global__ 
void Kerneltest(cufftDoubleComplex **da, unsigned int rows, unsigned int cols)
{
	unsigned int row = blockDim.y*blockIdx.y + threadIdx.y;
	unsigned int col = blockDim.x*blockIdx.x + threadIdx.x;
	if (row < rows && col < cols)
	{
		da[row][col].x = row*cols + col;
        da[row][col].y = row*cols + col;
	}
    // printf("%d  %d  %f+(%f)i\n",row,col,da[row][col].x,da[row][col].y);
}
 
__global__ 
void matrix_WRWmul_gpu(cufftDoubleComplex *A_Comp, cufftDoubleComplex *R_sigdata, double *T_sence_mo)  // W_CBF' x R_sig x W_CBF
{
    ...
    // 亮温反演
    // printf("i=%d,j=%d\n",i,j);
    printf("%d  %d  %f+(%f)i\n",i,j,A_Comp[20].x,R_sigdata[20].y);
    // 计算第一步乘结果
    ...
    for (int k = 0; k < Na; k++)        // 中间结果索引k
    {
        ...
        {
            // T_sence_left[k].x += (A_Comp[k2*Nay_net*Na+j*Nay_net+i].x * R_sigdata[k2*Na+k].x + \
            //                       A_Comp[k2*Nay_net*Na+j*Nay_net+i].y * R_sigdata[k2*Na+k].y);     // [i,j]网格处
            // T_sence_left[k].y += (A_Comp[k2*Nay_net*Na+j*Nay_net+i].x * R_sigdata[k2*Na+k].y - \
            //                       A_Comp[k2*Nay_net*Na+j*Nay_net+i].y * R_sigdata[k2*Na+k].x);     // [i,j]网格处
            T_sence_left[k].x += (A_Comp[1].x * R_sigdata[1].x + A_Comp[1].y * R_sigdata[1].y);     // [i,j]网格处
            T_sence_left[k].y += (A_Comp[1].x * R_sigdata[1].y - A_Comp[1].y * R_sigdata[1].x);     // [i,j]网格处
        }
        // printf("%d  %d  %f+(%f)i\n",i,j,T_sence_left[k].x,T_sence_left[k].y);
    }
    ...
    for (int k = 0; k < Na; k++)        // 中间结果索引k
    {
        T_sence.x += (T_sence_left[k].x * A_Comp[k*Nay_net*Na+j*Nay_net+i].x - \
                      T_sence_left[k].y * A_Comp[k*Nay_net*Na+j*Nay_net+i].y);
        T_sence.y += (T_sence_left[k].x * A_Comp[k*Nay_net*Na+j*Nay_net+i].y + \
                      T_sence_left[k].y * A_Comp[k*Nay_net*Na+j*Nay_net+i].x);   
        T_sence_mo[j*Nay_net+i] = sqrt(T_sence.x * T_sence.x + T_sence.y * T_sence.y); // 取模
...
int main()
{
    ...
    // 为二维数组在显存中分配内存
    cufftDoubleComplex *d_dataA;
    cudaMalloc ((void**)&d_dataA, sizeof(cufftDoubleComplex)*Naz_net*Nay_net*Na);
    cufftDoubleComplex *d_dataB;
    cudaMalloc ((void**)&d_dataB, sizeof(cufftDoubleComplex)*Na*Na);
    double *d_dataC;
    cudaMalloc ((void**)&d_dataC, sizeof(double)*Naz_net*Nay_net);

    // 获取显存中的二维数组的 pitch 值
    size_t d_pitchA, d_pitchB, d_pitchC;

    // A: 权重因子A_Comp 240x(240x256)
    cudaMallocPitch ((void**) &d_dataA, &d_pitchA, sizeof(cufftDoubleComplex)*Nay_net*Na, Naz_net);
    // 将二维数组转移进显存
    cudaMemcpy2D(d_dataA, d_pitchA, A_Comp, sizeof(cufftDoubleComplex)*Nay_net*Na, \
                 sizeof(cufftDoubleComplex)*Nay_net*Na, Naz_net, cudaMemcpyHostToDevice);

    // B: 协方差矩阵 256x256 
    cudaMallocPitch ((void**) &d_dataB, &d_pitchB, sizeof(cufftDoubleComplex)*Na, Na);
    // 将二维数组转移进显存
    cudaMemcpy2D(d_dataB, d_pitchB, R_sigdata, sizeof(cufftDoubleComplex)*Na, \
                 sizeof(cufftDoubleComplex)*Na, Na, cudaMemcpyHostToDevice);

    dim3 threadPerBlock(24, 24);
    dim3 blockNumber((Col+threadPerBlock.x-1)/ threadPerBlock.x, (Row+threadPerBlock.y-1)/ threadPerBlock.y);
    printf("Block(%d,%d)   Grid(%d,%d).\n", threadPerBlock.x, threadPerBlock.y, blockNumber.x, blockNumber.y);

    // matrix_WRWmul_gpu << <blockNumber, threadPerBlock >> > (d_dataA, d_dataB, d_dataC);
    matrix_WRWmul_gpu << <1, 1 >> > (d_dataA, d_dataB, d_dataC);
    // Kerneltest<<<blockNumber, threadPerBlock>>>(d_dataB, Na, Na);
	cudaDeviceSynchronize();

    // C: 保存成像结果 240x240
    // 将二维数组从显存传输回主机端的结果矩阵中
    cudaMemcpy2D(T_sence_mo, sizeof(double)*Naz_net, d_dataC, d_pitchC, sizeof(double)*Naz_net, Nay_net, cudaMemcpyDeviceToHost);
    ...
    char c = '\n'; //定义换行转义字符
    FILE *fp; //定义文件指针
    if ((fp = fopen("ImagingResult.csv","w")) == NULL)
    {
